...
@section('section')
    <div class="p-4 bg-white block sm:flex items-center justify-between border-b border-gray-200 lg:mt-1.5 dark:bg-gray-800 dark:border-gray-700">
        <div class="w-full mb-1">
            <div class="mb-4">
                ...
                <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white">Tugas</h1>
            </div>
            <div class="sm:flex">
                ...
                <a href="{{ route('TugasAdmin.Create') }}">
                    <button type="button" class="inline-flex items-center justify-center w-1/2 px-3 py-2 text-sm font-medium text-center text-white rounded-lg bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 sm:w-auto dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800 me-4">
                        Tambahkan Tugas
                    </button>
                </a>
                    ...
                    <table class="min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                            ...
                            <tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
                                ...
                                <td class="max-w-sm p-4 overflow-hidden text-base font-normal text-gray-500 truncate xl:max-w-xs dark:text-gray-400">
                                <a href="{{ $item->urltugas }}">{{ $item->urltugas }}</a>
                                </td>
                                <td class="p-4 text-base font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    {{-- <img src="{{ asset('storage/' .  $item->urlbukti ) }}" width='50px' height='30px'> --}}
                                    {{ $item->hari }}
                                </td>
                                <td class="p-4 text-base font-medium text-gray-900 whitespace-nowrap dark:text-white flex">
                                    <form action="{{ route('TugasAdmin.Delete', ['id' => $item->id]) }}" method = "post" onclick="return confirm('Apakah Anda yakin ingin menghapus materi ini?')">
                                        @csrf
                                        @method("DELETE")
    ...
    {{-- <table>
        ...
        <tr>
            <td>{{ $item->judul }}</td>
            <td><a href="{{ $item->urltugas }}">{{ $item->urltugas }}</a></td>
            <td>{{ $item->hari }}</td>
            <td>
                <a href='{{ route('TugasAdmin.Edit', ['id' => $item->id]) }}' class="btn btn-warning btn-sm">Edit</a>
            
                <form action="{{ route('TugasAdmin.Delete', ['id' => $item->id]) }}" method = "post" onclick="return confirm('Apakah Anda yakin ingin menghapus Tugas ini?')">
                    @csrf
                    @method("DELETE")
                        <button type="submit" name="submit">Del</button>
                </form>
            </td>
        </tr>
        @endforeach
    ...
    <div class="mt-5">
        <button type="button" class="inline-flex items-center justify-center w-1/2 px-3 py-2 text-sm font-medium text-center text-white rounded-lg bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 sm:w-auto dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-800 ms-4">
